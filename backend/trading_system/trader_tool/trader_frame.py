...
class trader_frame:
    ...
    def get_trader_frame(self):
        if self.trader_tool=='ths':
            self.trader=xgtrader(exe=self.exe,tesseract_cmd=self.tesseract_cmd,open_set=self.open_set,slippage=self.slippage)
            return self.trader
        else:
            self.trader=qmt_trader_ths(path=self.qmt_path,account=self.qmt_account,account_type=self.qmt_account_type,slippage=self.slippage)
            return self.trader
