...
from .user_def_models_buy import user_def_models_buy
from .user_def_models_sell import user_def_models_sell
...
import time
class tdx_letter_plate_custom_algorithm_trading_strategy:
    def __init__(self,trader_tool='qmt',exe='C:/同花顺软件/同花顺/xiadan.exe',tesseract_cmd='C:/Program Files/Tesseract-OCR/tesseract',
                ...
                qmt_account='55009640',qmt_account_type='STOCK',name='run_tdx_letter_plate_custom_algorithm_trading_strategy'):
        '''
        通达信板块自定义算法交易策略
        '''
        self.exe=exe
        ...
        with open(r'{}\通达信板块自定义算法交易策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        text['持股限制']=hold_limit
        text['持有限制']=hold_limit
        #保存
        with open(r'{}\通达信板块自定义算法交易策略.json'.format(self.path),'w',encoding='utf-8') as f:
    ...
    def save_position(self):
        ...
        if df.shape[0]>0:
            df['选择']=df['证券代码'].apply(lambda x: self.trader.select_data_type(x))
            df=df[df['选择']=='stock']
            df=df[df['股票余额']>=10]
            print('剔除黑名单**********')
    ...
    def save_balance(self):
        ...
        return df
    def get_read_tdx_data(self,name, path):
        ''' 读取通达信数据板块 '''
        try:
            stock_list = []
            ...
            df['代码'] = df['证券代码'].apply(lambda x: select_stock(x))
            df['证券代码']=df['代码'].apply(lambda x: str(x)[:6])
            print('{}板块读取成功'.format(name))
           
        except Exception as e:
            print(e, '{}板块识别 通达信路径有问题可能不存在'.format(name), path)
            df = pd.DataFrame()
        print(df)
        return df
    def get_read_all_tdx_data(self,name_list=[],path_list=['']):
        ''' 读取通达信多个模块成分股 '''
        data = pd.DataFrame()
        for name,path in zip(name_list,path_list):
            df = self.get_read_tdx_data(name=name,path=path)
    ...
    def  get_trader_stock_data(self):
        ...
        data_type=text['股票池']
        name_list=text['板块名称']
        path_list=text['通达信板块路径']
        if data_type=='通达信':
            data=self.get_read_all_tdx_data(name_list=name_list,path_list=path_list)
            print('通达信板块数据*************************888')
            print(data)
            data.to_excel(r'{}\自定义交易股票池\自定义交易股票池.xlsx'.format(self.path))
        else:
            print('自定义股票池数据****************')
            data=pd.read_excel(r'{}\自定义交易股票池\自定义交易股票池.xlsx'.format(self.path))
            print(data)
            ...
            if df.shape[0]>0:
                ...
                for i in tqdm(range(len(stock_list))):
                    ...
                    try: 
                        ...
                        models=user_def_models_buy(df=hist) 
        ...
        if is_open=='是':
            ...
            if df.shape[0]>0:
                ...
                for i in tqdm(range(len(stock_list))):
                    ...
                    try: 
                        hist=self.data.get_hist_data_em(stock=stock)     
                        models=user_def_models_sell(df=hist) 
                        for name in  factor_name:
                            try:
                                func=user_factor[name] 
                                func='models.{}'.format(func)
    ...
    def get_time_rotation(self):
        ...
        轮动方式
        '''
        with open('{}/通达信板块自定义算法交易策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
