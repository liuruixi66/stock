...
class small_fruit_fund_popularity_six_pulse_excalibur_trend_enhancement_strategy:
    def __init__(self,trader_tool='ths',exe='C:/同花顺软件/同花顺/xiadan.exe',tesseract_cmd='C:/Program Files/Tesseract-OCR/tesseract',
                ...
                qmt_account='55009640',qmt_account_type='STOCK',name='run_global_major_asset_six_pulse_excalibur_rotation_strategy'):
        '''
        小果同花顺基金人气六脉神剑趋势增强策略
        '''
        self.exe=exe
        ...
        with open(r'{}\小果同花顺基金人气六脉神剑趋势增强策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        text['持股限制']=hold_limit
        text['持有限制']=hold_limit
        #保存
        with open(r'{}\小果同花顺基金人气六脉神剑趋势增强策略.json'.format(self.path),'w',encoding='utf-8') as f:
    ...
    def cacal_mean_analysis_models(self):
        ...
        if df1.shape[0]>0:
            ...
            hold_stock_list=[]
        trader_df=pd.read_excel(r'{}\自定义交易股票池\自定义交易股票池.xlsx'.format(self.path),dtype='object')
        trader_df['标记1']=trader_df['名称'].apply(lambda x: str(x)[:2])
        trader_df['标记2']=trader_df['名称'].apply(lambda x: str(x).split('ETF')[0][-2:])
        trader_df=trader_df.drop_duplicates(subset=['标记1'])
        trader_df=trader_df.drop_duplicates(subset=['标记2'])
        if is_open=='是':
            print('开启价格站上N日线买入*****************')
            if trader_df.shape[0]>0:
                print('原始自定义交易股票池****************')
    ...
    def get_deal_etf(self):
        ...
        剔除一样ETF
        '''
        df=pd.read_excel(r'{}\自定义选股模型\自定义选股模型.xlsx'.format(self.path))
        if df.shape[0]>0:
