...
import time
class etf_custom_algorithmic_trading_system_strategy:
    def __init__(self,trader_tool='ths',exe='C:/同花顺软件/同花顺/xiadan.exe',tesseract_cmd='C:/Program Files/Tesseract-OCR/tesseract',
        ...
        with open(r'{}\etf自定义算法交易系统.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        text['持股限制']=hold_limit
        text['持有限制']=hold_limit
        #保存
        with open(r'{}\etf自定义算法交易系统.json'.format(self.path),'w',encoding='utf-8') as f:
    ...
    def get_trader_stock(self):
        ...
        if stock_soure=='人气排行ETF':
            ...
            print(df)
            '''
            info,df=data.get_user_def_data(func=func)
    ...
    def get_time_rotation(self):
        ...
        轮动方式
        '''
        with open('{}/etf自定义算法交易系统.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
