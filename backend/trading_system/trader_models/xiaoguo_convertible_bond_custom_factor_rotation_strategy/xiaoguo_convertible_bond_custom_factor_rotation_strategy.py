...
class xiaoguo_convertible_bond_custom_factor_rotation_strategy:
    def __init__(self,trader_tool='ths',exe='C:/同花顺软件/同花顺/xiadan.exe',tesseract_cmd='C:/Program Files/Tesseract-OCR/tesseract',
                ...
                qmt_account='55009640',qmt_account_type='STOCK',name='run_xiaoguo_convertible_bond_custom_factor_rotation_strategy'):
        '''
        小果可转债自定义因子轮动策略        
        '''
        self.exe=exe
        ...
        self.name=name
        with open(r'{}/小果可转债自定义因子轮动策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        self.url=text['服务器']
        self.port=text['端口']
        ...
        #保存
        with open(r'{}\小果可转债自定义因子轮动策略.json'.format(self.path),'w',encoding='utf-8') as f:
    ...
    def get_cacal_factor_base_table(self):
        ...
        '''
        print('计算默认因子***********************')
        with open('{}/小果可转债自定义因子轮动策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
