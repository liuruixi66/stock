...
import time
class bond_custom_algorithmic_trading_system_strategy:
    def __init__(self,trader_tool='ths',exe='C:/同花顺软件/同花顺/xiadan.exe',tesseract_cmd='C:/Program Files/Tesseract-OCR/tesseract',
                ...
                qmt_account='55009640',qmt_account_type='STOCK',name='run_bond_custom_algorithmic_trading_system_strategy'):
        '''
        可转债自定义算法交易交易系统策略
        '''
        self.exe=exe
        ...
        with open(r'{}\可转债自定义算法交易系统.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        text['持股限制']=hold_limit
        text['持有限制']=hold_limit
        #保存
        with open(r'{}\可转债自定义算法交易系统.json'.format(self.path),'w',encoding='utf-8') as f:
    ...
    def get_bond_data(self):
        ...
        port=text['端口']
        password=text['授权码']
        jsl_account=text['集思录账户']
        jsl_password=text['集思录密码']
        now_date=str(datetime.now())[:10]
        if data_type=='服务器':
            try:
                print('采用小果服务器更新可转债数据*******************')
                xg_data=xg_bond_data_down(url=url,port=port,password=password)
                df=xg_data.xg_bond_data_down(date=now_date)
                stats=df['数据状态'].tolist()[-1]
                if stats==True:
                    print('{}数据更新完成**********'.format(now_date))
                    df.to_excel(r'{}\自定义交易股票池\自定义交易股票池.xlsx'.format(self.path))
                    print(df)
                    #print(str(df.columns.tolist()).replace("'",'"'))
                else:
                    print('{}数据更新失败**********'.format(now_date))
                    print(df)
                    df=pd.DataFrame()
                    df.to_excel(r'{}\自定义交易股票池\自定义交易股票池.xlsx'.format(self.path))
            except Exception as e:
                print('服务器获取数据失败采用集思录数据')
                print('集思录获取数据******************')
                jsl=jsl_xg(user=jsl_account,password=jsl_password)
                jsl.login()
                df=jsl.get_conv_data_jsl()
                df['证券代码']=df['转债代码']
                df.to_excel(r'{}\自定义交易股票池\自定义交易股票池.xlsx'.format(self.path))
                print(df)
        else:
    ...
    def get_del_sell_stock(self):
        ...
        if sell_df.shape[0]>0:
            ...
            sell_df=sell_df[sell_df['交易品种']=='bond']
            sell_df.to_excel(r'{}\卖出股票\卖出股票.xlsx'.format(self.path))
            print('隔离符合交易的卖出股票池****************')
            print(sell_df)
        else:
    ...
    def get_time_rotation(self):
        ...
        轮动方式
        '''
        with open('{}/可转债自定义算法交易系统.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
