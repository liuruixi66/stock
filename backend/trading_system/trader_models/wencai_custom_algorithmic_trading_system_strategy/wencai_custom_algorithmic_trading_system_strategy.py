...
import time
class wencai_custom_algorithmic_trading_system_strategy:
    def __init__(self,trader_tool='qmt',exe='C:/同花顺软件/同花顺/xiadan.exe',tesseract_cmd='C:/Program Files/Tesseract-OCR/tesseract',
                ...
                qmt_account='55009640',qmt_account_type='STOCK',name='run_wencai_custom_algorithmic_trading_system_strategy'):
        '''
        问财自定义算法交易交易系统策略
        '''
        self.exe=exe
        ...
        with open(r'{}\问财自定义算法交易系统.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        text['持股限制']=hold_limit
        text['持有限制']=hold_limit
        #保存
        with open(r'{}\问财自定义算法交易系统.json'.format(self.path),'w',encoding='utf-8') as f:
    ...
    def get_trader_stock(self):
        ...
        port=text['端口']
        pasword=text['授权码']
        stock_soure=text['股票池']
        data=xg_data(url=url,port=port,password=pasword)
        if stock_soure=='问财选股条件':
            try:
                word=text['问财选股条件']
                models=wencai_threading_data(text=word)
                models.get_all_connect_wencai_data()
                df=models.get_result()
                df['证券代码']=df['代码']
                print('问财选股数据*************多线程选股*******************')
                print(df)
    ...
    def get_time_rotation(self):
        ...
        轮动方式
        '''
        with open('{}/问财自定义算法交易系统.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
