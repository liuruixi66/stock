...
class small_fruit_band_trading_strategy:
    def __init__(self,trader_tool='ths',exe='C:/同花顺软件/同花顺/xiadan.exe',tesseract_cmd='C:/Program Files/Tesseract-OCR/tesseract',
        ...
        with open(r'{}\小果波段交易策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        text['持股限制']=hold_limit
        text['持有限制']=hold_limit
        #保存
        with open(r'{}\小果波段交易策略.json'.format(self.path),'w',encoding='utf-8') as f:
    ...
    def get_connect_all_factor(self):
        '''
        链接全部默认因子，后面可以服务器提供
        '''
        print('链接全部默认因子，后面可以服务器提供')
        trader_df=pd.read_excel(r'{}\自定义交易股票池\自定义交易股票池.xlsx'.format(self.path),dtype='object')
        trader_df['证券代码']=trader_df['证券代码'].astype(str)
        stock_list=trader_df['证券代码'].tolist()
        df=self.dfcf_etf_data.get_all_etf_data_1()
        df['基金代码']=df['基金代码'].astype(str)
        df['证券代码']=df['基金代码']
        df['选择']=df['证券代码'].apply(lambda x: '是' if x in stock_list else '不是')
        df=df[df['选择']=='是']
        df.to_excel(r'{}\默认因子\默认因子.xlsx'.format(self.path))
        return df
    def get_user_def_func_models(self):
        ...
        df=pd.read_excel(r'{}\默认因子\默认因子.xlsx'.format(self.path))
        try:
            del df['Unnamed: 0']
        except:
            pass
            ...
            if df.shape[0]>0:
                ...
                for i in tqdm(range(len(stock_list))):
                    ...
                    try: 
                        ...
                        models=user_def_factor(df=hist) 
    ...
    def get_user_analysis_factor(self):
        ...
        if df.shape[0]>0:
            ...
            print('没有因子数据************')
            df.to_excel(r'{}\自定义因子数据选择\自定义因子数据选择.xlsx'.format(self.path))
    def get_connect_hold_stock_factor(self):
        '''
        链接全部持股的默认因子数据        
        '''
        print('链接全部持股的默认因子数据')
        ...
        df['选择']=df['证券代码'].apply(lambda x: '是' if x in stock_list else '不是')
        df=df[df['选择']=='是']
        df.to_excel(r'{}\持股默认因子\持股默认因子.xlsx'.format(self.path))
    def get_user_def_func_models_sell(self):
        ...
        factor_name=list(user_factor.keys())
        df=pd.read_excel(r'{}\持股默认因子\持股默认因子.xlsx'.format(self.path))
        try:
            del df['Unnamed: 0']
        except:
            pass
    ...
    def get_buy_sell_stock_data(self):
        ...
        if hold_stock.shape[0]>0:
            ...
            hold_stock_list=[]
            hold_amount=0
        buy_df=pd.read_excel(r'{}\自定义因子数据选择\自定义因子数据选择.xlsx'.format(self.path))
        buy_df['交易状态']='未买'
    ...
    def update_all_data(self):
        ...
        if self.base_func.check_is_trader_date_1():
            ...
            self.get_connect_all_factor()
            self.get_user_def_func_models()
            self.get_user_analysis_factor()
            self.get_connect_hold_stock_factor()
