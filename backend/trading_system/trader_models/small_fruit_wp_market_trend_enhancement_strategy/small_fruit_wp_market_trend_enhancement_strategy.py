...
import time
class small_fruit_wp_market_trend_enhancement_strategy:
    def __init__(self,trader_tool='ths',exe='C:/同花顺软件/同花顺/xiadan.exe',tesseract_cmd='C:/Program Files/Tesseract-OCR/tesseract',
                ...
                qmt_account='55009640',qmt_account_type='STOCK',name='run_small_fruit_wp_market_trend_enhancement_strategy'):
        '''
        小果主流外盘基金四趋势模型增强策略
        '''
        self.exe=exe
        ...
        with open(r'{}\小果主流外盘基金四趋势模型增强策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        text['持股限制']=hold_limit
        text['持有限制']=hold_limit
        #保存
        with open(r'{}\小果主流外盘基金四趋势模型增强策略.json'.format(self.path),'w',encoding='utf-8') as f:
    ...
    def get_time_rotation(self):
        ...
        轮动方式
        '''
        with open('{}/小果主流外盘基金四趋势模型增强策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
