...
class small_kirin_trend_enhancement_strategy:
    def __init__(self,trader_tool='ths',exe='C:/同花顺软件/同花顺/xiadan.exe',tesseract_cmd='C:/Program Files/Tesseract-OCR/tesseract',
        ...
        with open(r'{}\小果麒麟趋势增强策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        text['持股限制']=hold_limit
        text['持有限制']=hold_limit
        #保存
        with open(r'{}\小果麒麟趋势增强策略.json'.format(self.path),'w',encoding='utf-8') as f:
            ...
            if trader_df.shape[0]>0:
                ...
                for i in tqdm(range(len(stock_list))):
                    ...
                    try:
                        ...
                        models=the_kirin_trend_line(df=hist)
                        models=models.the_kirin_trend_line()
                        stats=models['stats'].tolist()[-1]
                        stats_list.append(stats)
                    except Exception as e:
                        print(e,'{}买入分析有问题'.format(stock))
