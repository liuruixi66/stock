...
class micro_stock_cap_trend_trading:
    def __init__(self,trader_tool='qmt',exe='C:/同花顺软件/同花顺/xiadan.exe',tesseract_cmd='C:/Program Files/Tesseract-OCR/tesseract',
        ...
        with open(r'{}\小果微盘股强势增强策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
    ...
    def save_balance(self):
        ...
        return df
    def get_select_trader_stock(self):
        '''
        选择交易的股票
        '''
        with open(r'{}\小果微盘股强势增强策略.json'.format(self.path),encoding='utf-8') as f:
        ...
        if trader_df.shape[0]>0:
            trader_df['证券代码']=trader_df['证券代码'].apply(lambda x: '0'*(6-len(str(x)))+str(x))
            trader_df['证券代码']=trader_df['证券代码'].astype(str)
            trader_df['剔除']=trader_df['证券代码'].apply(lambda x: '是' if str(x)[:3] in del_list else '不是')
            trader_df=trader_df[trader_df['剔除']=='不是']
        ...
        if df.shape[0]>0:
            ...
            if len(factor_name)>0:
                ...
                if 'and' in select_select:
                    ...
                    print(df)
                df['证券代码']=df['证券代码'].apply(lambda x: '0'*(6-len(str(x)))+str(x))
                df['代码']=df['证券代码'].apply(lambda x: '0'*(6-len(str(x)))+str(x))
    ...
    def get_del_not_trader_stock(self):
        ...
        if buy_df.shape[0]>0:
            ...
            #隔离策略
            buy_df['证券代码']=buy_df['证券代码'].apply(lambda x: '0'*(6-len(str(x)))+str(x))
            buy_df['代码']=buy_df['证券代码'].apply(lambda x: '0'*(6-len(str(x)))+str(x))
    ...
    def update_all_data(self):
        ...
        if self.base_func.check_is_trader_date_1():
            ...
            self.read_trader_stock()
            self.get_solation_strategy()
            self.get_select_trader_stock()
