...
class fund_flush_popularity_six_pulse_excalibur_trend_enhancement_strategy:
    def __init__(self,trader_tool='ths',exe='C:/同花顺软件/同花顺/xiadan.exe',tesseract_cmd='C:/Program Files/Tesseract-OCR/tesseract',
                ...
                qmt_account='55009640',qmt_account_type='STOCK',name='run_custom_six_pulse_excalibur_strategy'):
        '''
        基金同花顺人气六脉神剑趋势增强策略
        '''
        self.exe=exe
        ...
        with open(r'{}\基金同花顺人气六脉神剑趋势增强策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        text['持股限制']=hold_limit
        text['持有限制']=hold_limit
        #保存
        with open(r'{}\基金同花顺人气六脉神剑趋势增强策略.json'.format(self.path),'w',encoding='utf-8') as f:
    ...
    def get_ths_rq_fund_data(self):
        ...
        try:
            data=ths_rq()
            df=data.get_etf_hot_rank()
            df['证券代码']=df['代码']
            stock_list=df['证券代码'].tolist()
            df1=self.dfcf_etf_data.get_all_etf_data_1()
            print(df1)
            df1['选择']=df1['基金代码'].apply(lambda x: '是' if x in stock_list else '不是')
            df=df1[df1['选择']=='是']
            df['证券代码']=df['基金代码']
            print(df)
        except Exception as e:
            print(e,'获取同花顺人气数据有问题')
            df=pd.DataFrame()
