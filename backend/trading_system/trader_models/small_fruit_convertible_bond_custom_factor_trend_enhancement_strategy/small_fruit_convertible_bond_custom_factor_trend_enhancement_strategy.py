...
import time
class small_fruit_convertible_bond_custom_factor_trend_enhancement_strategy:
    def __init__(self,trader_tool='ths',exe='C:/同花顺软件/同花顺/xiadan.exe',tesseract_cmd='C:/Program Files/Tesseract-OCR/tesseract',
        ...
        with open(r'{}\小果可转债自定义因子趋势增强策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        text['持股限制']=hold_limit
        text['持有限制']=hold_limit
        #保存
        with open(r'{}\小果可转债自定义因子趋势增强策略.json'.format(self.path),'w',encoding='utf-8') as f:
        ...
        if is_open=='是':
            ...
            if df.shape[0]>0:
                ...
                for i in tqdm(range(len(stock_list))):
                    ...
                    result_list=[]
                    try:
                        hist=self.data.get_hist_data_em(start_date='19990101',stock=stock)   
        ...
        if df.shape[0]>0:
            ...
            if len(factor_name)>0:
                ...
                if 'and' in select_select:
                    ...
                    print(df)
                try:
                    df=df.drop_duplicates(subset=['可转债代码'],keep='last')
                except:
    ...
    def get_time_rotation(self):
        ...
        轮动方式
        '''
        with open('{}/小果可转债自定义因子趋势增强策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        now_date=''.join(str(datetime.now())[:10].split('-'))
