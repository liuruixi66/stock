<template>
  <main class="terminal-page">
    ...
    <section class="positions-section">
      <div class="panel-heading">
        <div><span>TRADE BLOTTER</span><h2>模拟盘交易流水</h2></div>
        <button class="command ghost" @click="loadOrders">刷新</button>
      </div>
      <table v-if="orders.length">
        <thead><tr><th>时间</th><th>代码</th><th>方向</th><th>类型</th><th>数量</th><th>成交价</th><th>成交金额</th><th>佣金</th><th>税费</th><th>状态</th><th>说明</th></tr></thead>
        <tbody>
          <tr v-for="item in orders" :key="item.id">
            <td>{{ formatTime(item.created_at) }}</td>
            <td>{{ item.symbol }}</td>
            <td :class="item.side === 'BUY' ? 'up' : 'down'">{{ item.side === 'BUY' ? '买入' : '卖出' }}</td>
            <td>{{ item.order_type === 'MARKET' ? '市价' : '限价' }}</td>
            <td>{{ item.quantity }}</td>
            <td>{{ item.executed_price?.toFixed(2) ?? '--' }}</td>
            <td>{{ item.amount?.toFixed(2) ?? '--' }}</td>
            <td>{{ item.commission.toFixed(2) }}</td>
            <td>{{ item.tax.toFixed(2) }}</td>
            <td>{{ orderStatusText(item.status) }}</td>
            <td class="memo">{{ item.message || '--' }}</td>
          </tr>
        </tbody>
      </table>
      <p v-else class="empty-hint">还没有模拟成交，提交一笔订单即可留下记录。</p>
    </section>
...
function orderStatusText(value: string) {
  return { FILLED: '已成交', PENDING: '待成交', REJECTED: '已拒绝' }[value] || value
}
...
async function loadOrders() {
  if (!selectedAccountId.value) { orders.value = []; return }
  try { orders.value = (await paperTradingApi.getOrders(selectedAccountId.value)).data.data }
  catch (value) { showError(value) }
}
...
function syncMarketAccounts() {
  ...
  orders.value = []
  if (selectedAccountId.value) { loadAccount(); loadOrders() }
}
