...
// 定义格式化函数
const formatters = {
  'sum_buy': (value: number) => (value / 10000).toFixed(2) + '万',
  'sum_sell': (value: number) => (value / 10000).toFixed(2) + '万',
  'net_amount': (value: number) => {
    const val = value / 10000
    return (val > 0 ? '+' : '') + val.toFixed(2) + '万'
  }
}
...
const handleExport = () => {
  ...
  const url = `/api/stock-top/?${params.toString()}`
